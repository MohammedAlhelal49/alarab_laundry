...
#     @property
#     def _table_query(self):
#         # keep context filtering (same as base)
#         users = self.env.context.get('commission_user_ids', [])
#         if users:
#             users = self.env['res.users'].browse(users).exists()
#         teams = self.env.context.get('commission_team_ids', [])
...
#
#         # NOTE:
#         # - commission_share_ratio is computed only from invoice lines (account.move).
#         # - For non-invoice lines, share factor = 1.0 (no split).
#         # - Final commission = raw tier commission * commission_share_ratio
#         return f"""
# WITH {self.env['sale.commission.achievement.report']._commission_lines_query(users=users, teams=teams)},
# achievement AS (
#     SELECT
...
#         MAX(scpf.id) AS forecast_id,
#         MAX(scpf.amount) AS forecast,
#         /* Weighted share ratio (divide total commission by this participation factor) */
#         CASE WHEN SUM(achieved) > 0 THEN
#             SUM(
#                 achieved * (
#                     CASE
#                         WHEN cl.related_res_model = 'account.move'
#                         THEN 1.0 / (1 + (
#                             SELECT COUNT(*) FROM account_move_additional_user_rel rel
#                             WHERE rel.move_id = cl.related_res_id
#                         ))
#                         ELSE 1.0
#                     END
#                 )
#             ) / SUM(achieved)
#         ELSE 1.0 END AS commission_share_ratio
#     FROM commission_lines cl
#     JOIN sale_commission_plan_target era
#         ON cl.plan_id = era.plan_id
#        AND cl.date >= era.date_from
...
#         a.forecast,
#         a.forecast_id,
#         /* Raw tier commission, then divided by share factor */
#         (
#             CASE
#                 WHEN tc.before IS NULL THEN a.achieved
#                 WHEN tc.rate_high IS NULL THEN tc.before
#                 ELSE tc.before + (tc.amount - tc.before) * (a.achieved_rate - tc.rate_low) / NULLIF((tc.rate_high - tc.rate_low), 0)
#             END
#         ) * a.commission_share_ratio AS commission
#     FROM achievement a
#     LEFT JOIN target_com tc ON (
#         tc.plan_id = a.plan_id AND
#         tc.rate_low <= a.achieved_rate AND
