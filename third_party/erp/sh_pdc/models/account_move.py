...
class AccountInvoice(models.Model):
    ...
    pdc_payment_ids = fields.Many2many(
        'pdc.wizard', compute='_compute_pdc_payment_invoice',store=True)
    ...
    sh_pdc_status = fields.Selection([('draft', 'Draft'), ('registered', 'New'), ('returned', 'Returned'),
                              ('deposited', 'Registered'), ('bounced', 'Bounced'), ('done', 'Done'), ('cancel', 'Cancelled')], string="PDC Status",compute='_compute_latest_pdc_payment_status',store=True,search='_search_latest_pdc_payment_status')
    ...
    pdc_count = fields.Integer(
        compute='_compute_pdc_count'
    )

    def _compute_pdc_count(self):
        for rec in self:
            rec.pdc_count = 1 if rec.pdc_id else 0

    def action_open_pdc(self):
        self.ensure_one()
