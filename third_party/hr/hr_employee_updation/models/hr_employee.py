...
class HrEmployee(models.Model):
    ...
    def _onchange_identification_id(self):
        """Improvement: when Identification No is cleared, also clear its
        expiry date and attachments so no 'orphaned' expiry date remains
        stored for a document number that no longer exists. This mirrors
        the UI's invisible condition (id_expiry_date is only shown when
        identification_id is set) and keeps the underlying data consistent
        with what the user sees."""
        if not self.identification_id:
            self.id_expiry_date = False
            self.identification_attachment_ids = [(5, 0, 0)]

    @api.onchange('passport_id')
    def _onchange_passport_id(self):
        """Improvement: same consistency fix as _onchange_identification_id,
        but for the passport number/expiry date/attachments."""
        if not self.passport_id:
            self.passport_expiry_date = False
            self.passport_attachment_ids = [(5, 0, 0)]
        ...
        for employee in employee_ids:
            ...
            if employee.id_expiry_date:
                ...
                if current_date >= exp_date:
                    mail_content = (
                            "Hello %s,<br>Your ID %s is going to expire on %s."
                            " Please renew it before expiry date" % (
                                employee.name,
                                employee.identification_id or _('N/A'),
            ...
            if employee.passport_expiry_date:
                ...
                if current_date >= exp_date:
                    mail_content = (
                            "Hello %s,<br>Your Passport %s is going to expire"
                            " on %s. Please renew it before expire" % (
                                employee.name,
                                employee.passport_id or _('N/A'),
